...
import { adminIndividualListersSearchSchema } from '@/features/admin/schemas/moderation-schemas';

export async function GET(request: NextRequest) {
  const parsed = adminIndividualListersSearchSchema.safeParse(Object.fromEntries(request.nextUrl.searchParams));
  if (!parsed.success) {
  ...
  return proxyToBackend(request, {
    path: backendPaths.individualListers.list,
    method: 'GET',
    searchParams: params,
  });
}
