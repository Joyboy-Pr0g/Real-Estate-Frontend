...
const STATUS_STYLES: Record<MyListingReportStatus, string> = {
...
export function AdminListingReportsPanel({
  ...
  const [status, setStatus] = useState<MyListingReportStatus>('resolved');
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      const page = await fetchAdminListingReports(params);
  ...
  const openAnswerModal = (report: MyListingReport) => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const updated = await updateListingReport(activeReportId, {
        status,
    ...
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-[var(--shadow-soft)]">
          <p className="text-xs font-medium text-gray-400">{t('admin.listingReports.total')}</p>
          <p className="mt-1 text-2xl font-bold text-primary-dark">{totalReports}</p>
        </div>
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-[var(--shadow-soft)]">
          <p className="text-xs font-medium text-gray-400">{t('admin.listingReports.pending')}</p>
      ...
      {items.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-gray-200 bg-white px-6 py-16 text-center">
          <p className="text-gray-500">{t('admin.listingReports.empty')}</p>
        </div>
      ) : (
      ...
      {activeReport ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          ...
          >
            <h2 className="text-lg font-bold text-primary-dark">{t('admin.listingReports.answerTitle')}</h2>
            <p className="mt-1 truncate text-sm text-gray-500">{activeReport.listing_title}</p>
            ...
            <div className="mt-5 space-y-4">
              <label className="block space-y-1.5">
                ...
                <select
                  required
                  value={status}
                  onChange={(e) => setStatus(e.target.value as MyListingReportStatus)}
