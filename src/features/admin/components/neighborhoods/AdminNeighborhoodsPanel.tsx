...
import { NeighborhoodCard } from '@/features/admin/components/neighborhoods/NeighborhoodCard';
import { NeighborhoodFormDialog } from '@/features/admin/components/neighborhoods/NeighborhoodFormDialog';
import { NeighborhoodTable } from '@/features/admin/components/neighborhoods/NeighborhoodTable';
import {
  deleteNeighborhood,
  loadMoreAdminNeighborhoods,
...
interface AdminNeighborhoodsPanelProps {
  initial: AdminLocationsPage<AdminNeighborhood>;
...
export function AdminNeighborhoodsPanel({
  ...
  const [editItem, setEditItem] = useState<AdminNeighborhood | null>(null);
  const [deleteItem, setDeleteItem] = useState<AdminNeighborhood | null>(null);
  ...
  useEffect(() => {
    ...
    applyFilters(debouncedSearch, cityFilter);
  }, [debouncedSearch, cityFilter, applyFilters]);
  ...
  const handleLoadMore = async () => {
    ...
    setLoadingMore(true);
    try {
      const page = await loadMoreAdminNeighborhoods({
        cursor: nextCursor,
    ...
    <div className={cn('space-y-6 transition-opacity', isPending && 'opacity-60')}>
      <AdminPageHeader
        breadcrumbItems={[
          ...
          { labelKey: 'admin.neighborhoods', icon: MapPin },
        ]}
        title={t('admin.neighborhoods')}
      ...
      {items.length === 0 ? (
        <div className="rounded-2xl border border-gray-200 bg-white p-12 text-center shadow-[var(--shadow-soft)]">
          <p className="text-gray-500">{t('admin.noNeighborhoods')}</p>
        </div>
      ) : (
        <>
          <NeighborhoodTable
      ...
      {deleteItem ? (
        <ConfirmModal
          ...
          onConfirm={() => void runAction(deleteItem.id, () => deleteNeighborhood(deleteItem.id), t('admin.neighborhoodDeleted'))}
        />
      ) : null}
    </div>
  );
