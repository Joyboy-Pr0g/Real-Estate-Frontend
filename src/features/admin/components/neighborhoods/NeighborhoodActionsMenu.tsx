...
interface NeighborhoodActionsMenuProps {
  disabled?: boolean;
  onEdit: () => void;
  onDelete: () => void;
}

export function NeighborhoodActionsMenu({
  disabled = false,
  onEdit,
  onDelete,
}: NeighborhoodActionsMenuProps) {
  const { t } = useLocale();
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-48">
        {hasPermission('neighborhoods.edit') ? (
          <DropdownMenuItem className="cursor-pointer" onClick={onEdit}>
            {t('admin.edit')}
          </DropdownMenuItem>
        ) : null}
        {hasPermission('neighborhoods.delete') ? (
