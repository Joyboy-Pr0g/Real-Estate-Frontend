...
import { AdminPropertyType, PropertyTypePayload } from '@/features/admin/types/catalog';
...
export function PropertyTypeFormDialog({ open, item, onClose, onSaved }: PropertyTypeFormDialogProps) {
  const { t } = useLocale();
  const [pending, startTransition] = useTransition();
  const [form, setForm] = useState<PropertyTypePayload>({ name: '', icon: '' });
    ...
    startTransition(async () => {
      try {
        ...
        if (item) {
          ...
          await updatePropertyType(item.id, patch);
          toast.success(t('admin.propertyTypeUpdated'));
        } else {
          await createPropertyType(next);
          toast.success(t('admin.propertyTypeCreated'));
        }
        onSaved();
        onClose();
      } catch (err) {
      ...
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-[var(--shadow-float)]">
        <div className="relative">
          ...
          <h2 className="ps-10 text-lg font-bold text-primary-dark">
            {item ? t('admin.editPropertyType') : t('admin.createPropertyType')}
          </h2>
        </div>
